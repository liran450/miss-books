...
function query() {
//save the gBooks initially here
...
function removeReview(currBook, id){
    var idx = currBook.bookReviews.findIndex(review => review.id === id)
    if (idx === -1) return
    currBook.bookReviews.splice(idx, 1)
    return storageService.put(BOOKS_KEY, currBook)
}

(function saveOrGetBooksInStorage() {
    let books = utilService.loadFromStorage(BOOKS_KEY)
    if (!books || books.length === 0) {
        books = utilService.saveToStorage(BOOKS_KEY, gBooks)
    }
    gBooks = books
})();
